...
class InfixSpecInfoExtractorImplTest extends UnitTestSuite {
  ...
  private val infixSpecInfoExtractor = new InfixSpecInfoExtractorImpl(registrationWordClassifier, subjectInfoExtractor)
  ...
  test("extract() when valid and not ignored, should return correct spec name") {
    ...
    when(registrationWordClassifier.isSpecVerb(eqTree(q"should"))).thenReturn(true)
  ...
  test("extract() when verb is invalid should return None") {
    ...
    when(registrationWordClassifier.isSpecVerb(eqTree(q"blabla"))).thenReturn(false)
