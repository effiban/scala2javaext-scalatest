...
class MatcherAssertionTransformerImplTest extends UnitTestSuite {
  ...
  private val matcherAssertionTransformer = new MatcherAssertionTransformerImpl(matcherVerbClassifier, matcherTransformer)
  ...
  test("transform() when verb is equality matcher verb, and matcher is transformed") {
    ...
    when(matcherVerbClassifier.isEqualityMatcherVerb(eqTree(verb))).thenReturn(true)
  ...
  test("transform() when verb is not equality matcher verb, and matcher is transformed") {
    ...
    when(matcherVerbClassifier.isEqualityMatcherVerb(eqTree(verb))).thenReturn(false)
