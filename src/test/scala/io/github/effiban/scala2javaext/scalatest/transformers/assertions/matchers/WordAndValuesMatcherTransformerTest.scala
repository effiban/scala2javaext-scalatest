...
class WordAndValuesMatcherTransformerTest extends UnitTestSuite {
  ...
  test("transform for a Term.Apply with qualified word 'valid' and expected args, should return expected matcher") {
  ...
  test("transform for a Term.Apply with qualified word 'invalid' should return None") {
    val matcher = q"super.invalid(3, 4)"

    TestMatcherTransformer.transform(matcher) shouldBe None
  }

  test("transform for a Term.Apply with unqualified word 'valid' and expected args, should return expected matcher") {
